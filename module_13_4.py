...
api = '7676529448:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA'
bot = Bot(token=api)
...
async def set_growth(message: types.Message, state: FSMContext):
    await state.update_data(age=int(message.text))      # Обновление данных в состоянии age на message.text
...
async def set_weight(message: types.Message, state: FSMContext):
    await state.update_data(growth=int(message.text))   # Обновление данных в состоянии growth на message.text
                                                        # (написанное пользователем сообщение)
